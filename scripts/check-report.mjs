...
const guide = 'manual/PROJECT_GUIDE.md';
...
assert(md.includes('**Maker: Bui Xuan Mai**') && md.includes(repo) && md.includes(prompts));
...
const diagramPath = 'assets/manual/diagrams/manifest.json';
...
const record = JSON.parse(read('assets/manual/report-build.json'));
...
assert.equal(record.pdfSha256, hash('manual/project-guide.pdf'), 'PDF bytes differ from build record');
assert.equal(record.diagramManifestSha256, hash(diagramPath), 'Regenerate PDF after rendering changed diagrams');
assert.equal(record.bytes, read('manual/project-guide.pdf').length);
assert.equal(record.maker, 'Bui Xuan Mai');
assert.equal(record.repository, repo);
assert.equal(record.promptReference, prompts);
assert(read('manual/project-guide.pdf').subarray(0, 5).equals(Buffer.from('%PDF-')));
...
for (const file of [guide, 'manual/project-guide.pdf', diagramPath, 'assets/manual/report-build.json',
...
console.log('Report checks passed: 15 linked chapters, 4 matching Mermaid figures, attribution, resources and PDF hashes (' + record.pages + ' pages).');
